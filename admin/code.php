...
{
  echo "<script>if(confirm('Email Already Exist!!')){document.location.href='add_staff.php'};</script>";
...
else
{
        ...
        $result =mysqli_query($con,"INSERT INTO `manager_cred`(`sr_no`, `email`, `name`, `number`, `address`, `manager_pass`, `role`,`verify_token`, `status`, `lg_status`) VALUES (DEFAULT,'$email','$name',' $number','$address','$manager_pass','STAFF','$verify_token',1,'STAFF')");
        if ($result){

        echo "<script>if(confirm('Registration Success!!')){document.location.href='add_staff.php'};</script>";
      
    }
    else
    {
        echo "<script>if(confirm('Registration failed....!!!')){document.location.href='add-staff.php'};</script>";
    }    
    }          
}
